...
log stream --predicate 'composedMessage contains "AVFCapture"' | while read line; do
  ...
  if echo "$line" | grep -qE "AVCaptureSession_Tundra startRunning"; then
	...
	curl --location --request PUT 'http://10.0.2.17:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":20,"temperature":150,"on":1}],"numberOfLights":1}' && echo

	#Light 2 (remove if you only have 1 light), change IP to your second light
	#curl --location --request PUT 'http://192.168.86.38:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":20,"temperature":150,"on":1}],"numberOfLights":1}'
  ...
  if echo "$line" | grep -qE "AVCaptureSession_Tundra stopRunning"; then
	...
	#Light 1 - set to off
	curl --location --request PUT 'http://10.0.2.17:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":20,"temperature":150,"on":0}],"numberOfLights":2}' && echo

	#Light 2 - set to off
	#curl --location --request PUT 'http://192.168.86.38:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":20,"temperature":150,"on":0}],"numberOfLights":2}'
