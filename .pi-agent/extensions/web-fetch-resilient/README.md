...
- **Success is judged after extraction, not before**: a tier only counts if the *answer* is substantive. A JS application shell is a large, well-formed 200 that Readability reduces to a title and nothing else — Reddit returned `200 · complete` with an empty body until this existed. Size is half the judgement: example.com is 1.2KB of HTML and 167 characters of prose, and that is the whole page, so only documents ≥ 8KB are suspicious for yielding < 200 characters. If every tier extracts nothing, the fullest attempt is returned with an explicit `WARNING` rather than thrown away.
