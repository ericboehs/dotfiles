...
// Compaction without the thinking tax.
//
...
// thinking level into that call, and github-copilot/claude-opus-5 is registered
// with `compat.forceAdaptiveThinking`, so at xhigh the model can spend the whole
...
//
//   1. Summarizes with thinking explicitly disabled, on the same model.
//   2. Gives the summary a real ceiling (32k) instead of 0.8 * reserveTokens.
//
...
/** Ceiling for the summary itself. Only emitted tokens are billed. */
const MAX_SUMMARY_TOKENS = 32_000;
...
export default function (pi: ExtensionAPI) {
	pi.on("session_before_compact", async (event, ctx) => {
		...
		// pi only emits `thinking: {type: "disabled"}` when the model is not pinned
		// thinking-only. That opt-out is spelled `off: null` in thinkingLevelMap
		// (claude-fable-5 has it; claude-opus-5 does not). Without this check the
		// request would silently fall through to the provider default and nothing
		// would change.
		...
		try {
			const response = await ctx.modelRegistry.complete(
				...
				{
					// The entire point. Omitting this leaves the param off the request
					// and lets the provider's adaptive-thinking default apply.
